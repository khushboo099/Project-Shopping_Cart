...
const createUser= async function(req, res) {
    ...
    return res.status(400).send({status: false, message: "shipping address should be with street, city and pincode"})
    ...
    return res.status(400).send({status: false, message: "billing address should be with street, city and pincode"})
    ...
    if(validString(data.fname) ||validString(data.lname) ) 
...
const loginUser = async (req, res) => {
    try{
        ...
        return res.status(400).send({status: false, message: "Email and Password is required for login"})

        //check email or password is present in body or not
        if(!data.email) 
        return res.status(400).send({status: false, message: "Email field is empty"})
        if(!data.password) 
        return res.status(400).send({status: false, message: "Password field is empty"})
...
const getProfile = async function (req, res) {
    try {
        ...
        if(!userId) 
        return res.status(400).send({status: false , message: "userId must be required"})
