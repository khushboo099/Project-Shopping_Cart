...
const createProduct= async function(req, res) {
    ...
    return res.status(400).send({status: false, message: "title should be characters and should not contains any numbers"})
    ...
    return res.status(400).send({status: false, message: " valid description is required"})
    ...
    return res.status(400).send({status: false, message: "Price is accept both number and decimal"})

    //check for currecyId
    if(!data.currencyId) 
    return res.status(400).send({status: false, message: "currencyId is required"})
     if(data.currencyId !=="INR") 
     return res.status(400).send({status: false, message: " valid currencyId i.e..,INR is required"})

     //check for currency format
    if(!data.currencyFormat) 
    return res.status(400).send({status: false, message: "currency format  is required"})
    if(data.currencyFormat!=="₹") 
    return res.status(400).send({status: false, message: " valid currencyformat i.e., ₹ is required"})
...
const getProduct = async(req, res) => {
    try {
        ...
        if(data?.size || typeof data.size == 'string'){
            ...
            return res.status(400).send({status: false, message: "Size should be one of S,XS,M,X,L,XXL,XL"})
        ...
        if(data?.name){
            ...
            return res.status(400).send({status: false, message: "Enter the value of products in name"})
        ...
        if(typeof data.priceGreaterThan == 'string') {
          if(!validString(data.priceGreaterThan)) 
          return res.status(400).send({ status: false, message: "Price of product should be in numbers" });
          
          data.priceGreaterThan = JSON.parse(data.priceGreaterThan);
          if(!(/^[0-9]*[1-9]+$|^[1-9]+[0-9]*$/.test(data.priceGreaterThan))) 
          return res.status(400).send({ status: false, message: "Price of product should be valid" });
      ...
      //validating the filter - PRICELESSTHAN
        ...
        if(typeof data.priceLessThan == 'string') {
          if(!validString(data.priceLessThan)) 
          return res.status(400).send({ status: false, message: "Price of product should be in numbers" });
          
          data.priceLessThan = JSON.parse(data.priceLessThan);
          if(!(/^[0-9]*[1-9]+$|^[1-9]+[0-9]*$/.test(data.priceLessThan))) 
          return res.status(400).send({ status: false, message: "Price of product should be valid" });
...
const getProductById = async function (req, res) {
  ...
  const updateProduct = async(req, res) => {
      try{
        ...
        return res.status(404).send({ status: false, message: "No products found or might have already been deleted" })
        ...
        if(isValidBody(data)) 
        return res.status(400).send({ status: false, message: "Enter data to update product" });
       ...
       return res.status(400).send({ status: false, message: "Action forbidden" });
        ...
        let checkUniqueTitle = await productModel.findOne({title: data.title})
        if(checkUniqueTitle) 
        return res.status(400).send({status: false, message: `${data.title} title is already exist `})

    
        if(data?.description) {
          //checking for product description
          if(!isValid(data.description) && validString(data.description)) 
          return res.status(400).send({ status: false, message: "Description should not be an empty string or any numbers in it" });
        };
    
        if(data?.price) {
          //checking for product price
          if((!validString(data.price) && validPrice(data.price))) 
          return res.status(400).send({ status: false, message: "Price of product should be valid and in numbers" });
        }
        ...
        if(data?.isFreeShipping) {
          ...
          return res.status(400).send({ status: false, message: "Free shipping should be in boolean value" })
        }
    
        //checking for style in data
        if(data?.style){
          if(isValid(data.style) && validString(data.style)) 
          return res.status(400).send({ status: false, message: "Style should be valid an does not contain numbers" });
        }
        ...
        if(data?.availableSizes) {
          //checking for available Sizes of the products
          if(isValid(data.availableSizes) && validString(data.availableSizes))  
          return res.status(400).send({ status: false, message: "Enter at least one available size" });
        ...
        if(data?.installments) {
          if(!validString(data.installments)) 
          return res.status(400).send({ status: false, message: "Installments should be in numbers" });
          if(validPrice(data.installments)) 
